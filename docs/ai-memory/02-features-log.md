...
- **Registration Closure**: Added a `REGISTRATIONS_CLOSED` flag inside `DigitalIndiaGSAPForm.tsx` that replaces the multi-step registration forms with a clean, luxury themed 'Registrations Closed' message container, while keeping referral status tracking and leaderboards fully functional.
