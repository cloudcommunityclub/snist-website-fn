...
export default function Countdown({ 
    ...
    return (
        <div className="flex flex-col items-center gap-2">
            {title && (
                <h3 className="title-main text-lg text-gray-300">{title}</h3>
            )}
            <div className={`flex flex-row gap-x-1 sm:gap-x-2 xl:gap-x-4 ${className}`}>
                <div className='p-2 sm:p-3 xl:p-6 title-main text-center font-bold text-2xl bg-neutral-800 rounded-xl'>
                    {currentFormat.days}<p className='text-xs sm:text-sm text-neutral-400'>days</p>
                </div>
                <div className='p-2 sm:p-3 xl:p-6 title-main text-center font-bold text-2xl bg-neutral-800 rounded-xl'>
                    {currentFormat.hours}<p className='text-xs sm:text-sm text-neutral-400'>hours</p>
                </div>
                <div className='p-2 sm:p-3 xl:p-6 title-main text-center font-bold text-2xl bg-neutral-800 rounded-xl'>
                    {currentFormat.minutes}<p className='text-xs sm:text-sm text-neutral-400'>mins</p>
                </div>
                <div className='p-2 sm:p-3 xl:p-6 title-main text-center font-bold text-2xl bg-neutral-800 rounded-xl'>
                    {currentFormat.seconds}<p className='text-xs sm:text-sm text-neutral-400'>secs</p>
                </div>
            </div>
        </div>
    )
