...
export default function DigitalIndiaGSAPForm() {
    ...
    const fetchDashboardStats = async (emailToFetch: string) => {
        ...
        try {
            const res = await fetch(
                `/api/digital-india/referrals/stats?email=${encodeURIComponent(emailToFetch.trim())}`
            )
    ...
    const fetchLeaderboard = async (search = '') => {
        ...
        try {
            const res = await fetch(
                `/api/digital-india/referrals/leaderboard?search=${encodeURIComponent(search)}`
            )
            const result = await res.json()
            if (res.ok && result.success) {
    ...
    useEffect(() => {
        function handleClickOutside(event: MouseEvent) {
            if (
                teamSizeDropdownRef.current &&
                !teamSizeDropdownRef.current.contains(event.target as Node)
            ) {
    ...
    useEffect(() => {
        ...
        const handleMouseMove = (e: MouseEvent) => {
            ...
            span.innerText = char
            span.className =
                'absolute pointer-events-none font-mono text-[#9dff00] font-bold select-none text-shadow-neon'
            ...
            const travelY =
                -Math.abs(Math.sin((angle * Math.PI) / 180) * distance) - 20 // Drift upwards
    ...
    useEffect(() => {
        const updateLinePosition = () => {
            ...
            const firstDot = document.querySelector(
                '.timeline-item-anim:first-child .timeline-dot'
            )
            const lastDot = document.querySelector(
                '.timeline-item-anim:last-child .timeline-dot'
            )
            ...
            if (isActive) {
                ...
                gsap.fromTo(
                    ref.current.querySelectorAll('.fade-in-content'),
                    {
                        ...
                        y: isFirstRender.current ? 0 : 15,
                    },
                    {
                        opacity: 1,
                        y: 0,
                        duration: innerDuration,
                        stagger: isFirstRender.current ? 0 : 0.08,
                        delay: innerDelay,
                        ease: 'power3.out',
                    }
                )
        ...
        if (teamSize > 1) {
            teamMembers.forEach((m, idx) => {
                if (!m.name || !m.name.trim()) {
                    newErrors[`teamMemberName_${idx}`] =
                        `Member ${idx + 2} name is required`
                }
                if (!m.email || !m.email.trim()) {
                    newErrors[`teamMemberEmail_${idx}`] =
                        `Member ${idx + 2} email is required`
                } else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(m.email)) {
                    newErrors[`teamMemberEmail_${idx}`] =
                        `Member ${idx + 2} has an invalid email`
                }
            })
        }
    ...
    const validateStep3 = (): boolean => {
        ...
        if (!idea.trim()) {
            ...
            newErrors.idea =
                'Please describe your idea in at least 150 characters'
        }
    ...
    const validateStep4 = (): boolean => {
        ...
        if (!utrId.trim()) {
            ...
            newErrors.utrId =
                'UTR / Transaction ID must be 10 to 35 alphanumeric characters'
        }
            ...
            <div className='h-full flex flex-col justify-between items-center py-8 w-full font-sans select-none'>
                ...
                <div
                    className={`text-xs font-bold font-mono transition-colors duration-300 ${
                        isCompleted ? 'text-[#9dff00]' : 'text-zinc-500'
                    }`}
                >
                    {stepNumber}
                </div>
                ...
                <div
                    className={`flex flex-col items-center gap-1.5 text-[9px] uppercase font-bold tracking-widest font-mono mt-auto transition-colors duration-300 ${
                        isCompleted ? 'text-[#9dff00]' : 'text-zinc-400'
                    }`}
                >
    ...
    return (
        <div
            ref={pageRef}
            className='min-h-screen relative flex flex-col overflow-hidden bg-[#09090b] text-[#f4f4f5] font-sans selection:bg-[#9dff00]/40'
        >
            ...
            <section
                ref={heroRef}
                className='relative overflow-hidden z-10 w-full max-w-7xl mx-auto min-h-[95vh] flex flex-col lg:flex-row items-center justify-between px-6 md:px-12 pt-36 pb-16 gap-12'
            >
                <div className='flex-1 text-left max-w-3xl'>
                    <h1 className='hero-animate-title text-5xl sm:text-7xl font-extralight text-white tracking-tight leading-[1.05] mb-6'>
                        Digital{' '}
                        <span className='font-normal text-[#9dff00] block sm:inline'>
                            INDIA.
                        </span>{' '}
                    ...
                    <p className='hero-animate-title text-zinc-400 text-base sm:text-lg font-light leading-relaxed mb-8 max-w-2xl'>
                        A national innovation challenge where participants
                        develop technology-driven solutions to real-world
                        problems faced by millions of Indians—from local
                        shopkeepers and street vendors to rural communities and
                        public services.
                    </p>
                    ...
                    <div className='hero-animate-title bg-zinc-900/35 border border-zinc-800/60 rounded-2xl p-4 mb-8 text-xs sm:text-sm font-light text-zinc-300 flex items-start gap-3 max-w-xl'>
                        <span className='text-[#9dff00] font-bold text-lg leading-none'>
                            ★
                        </span>
                        <div>
                            ...
                            The Top 100 most impactful problem statements will
                            be shortlisted.
                        </div>
                    </div>
                    ...
                    <div className='hero-animate-title grid grid-cols-2 sm:grid-cols-4 gap-6 mt-16 pt-8 border-t border-zinc-800/40'>
                        <div>
                            <div className='text-3xl font-light text-[#9dff00] font-mono'>
                                ₹2.5L
                            </div>
                            <div className='text-zinc-500 text-[10px] font-mono uppercase tracking-wider mt-1'>
                                Prize Pool
                            </div>
                        </div>
                        <div>
                ...
                <div className='flex-1 flex flex-col items-center justify-center w-full'>
                    ...
                    >
                        ...
                        <svg
                            viewBox='0 0 500 500'
                            className='w-full h-full drop-shadow-[0_0_40px_rgba(157,255,0,0.08)]'
                        >
                            <defs>
                                <filter
                                    id='glow-neon'
                                ...
                                >
                                    <stop
                                        offset='0%'
                                        stopColor='rgba(157,255,0,0.4)'
                                    />
                                    <stop
                                        offset='100%'
                                        stopColor='rgba(63,63,70,0.1)'
                                    />
                                </linearGradient>
                            </defs>
                            ...
                            <g filter='url(#glow-neon)'>
                                ...
                                <circle r='3' fill='#9dff00'>
                                    <animateMotion
                                        dur='2.8s'
                                        repeatCount='indefinite'
                                        path='M 250,250 L 340,250 L 400,160'
                                        begin='0.3s'
                                    />
                                </circle>
                                <circle r='2' fill='#9dff00' opacity='0.5'>
                                ...
                                <circle r='3' fill='#9dff00'>
                                    <animateMotion
                                        dur='2.6s'
                                        repeatCount='indefinite'
                                        path='M 250,250 L 340,250 L 400,340'
                                        begin='0.6s'
                                    />
                                </circle>
                                <circle r='2' fill='#9dff00' opacity='0.5'>
                                ...
                                <circle r='3' fill='#9dff00'>
                                    <animateMotion
                                        dur='2.9s'
                                        repeatCount='indefinite'
                                        path='M 250,250 L 160,250 L 100,340'
                                        begin='0.5s'
                                    />
                                </circle>
                                <circle r='2' fill='#9dff00' opacity='0.5'>
                                ...
                                <circle r='3' fill='#9dff00'>
                                    <animateMotion
                                        dur='2.7s'
                                        repeatCount='indefinite'
                                        path='M 250,250 L 160,250 L 100,160'
                                        begin='0.1s'
                                    />
                                </circle>
                                <circle r='2' fill='#9dff00' opacity='0.5'>
                            ...
                            />
                            <path
                                d='M 250,15 A 235,235 0 0,1 485,250'
                                fill='none'
                                stroke='#9dff00'
                                strokeWidth='1.5'
                            ...
                            />
                            <path
                                d='M 250,485 A 235,235 0 0,1 15,250'
                                fill='none'
                                stroke='#9dff00'
                                strokeWidth='1.5'
                            ...
                            <g>
                                ...
                                <g className='spin-ccw'>
                                    <circle
                                        ...
                                        r='30'
                                        fill='none'
                                        stroke='rgba(157, 255, 0, 0.3)'
                                        strokeWidth='1.5'
                                        strokeDasharray='15 15'
                                    />
                                    <path
                                        d='M 235,250 L 265,250 M 250,235 L 250,265'
                                        stroke='#9dff00'
                                        strokeWidth='1'
                                    />
                                </g>
            ...
            <section className='relative z-10 w-full max-w-7xl mx-auto px-6 md:px-12 py-24 border-t border-zinc-800/40'>
                <div className='text-center mb-16'>
                    <span className='text-xs font-mono text-[#9dff00] uppercase tracking-widest block mb-3'>
                        ABOUT THE HACKATHON
                    </span>
                    <h2 className='text-3xl md:text-5xl font-light text-white tracking-tight mb-4'>
                        More than a{' '}
                        <span className='text-[#9dff00] font-normal'>
                            Competition
                        </span>
                        , an{' '}
                    ...
                    <p className='text-zinc-400 text-sm md:text-base font-light max-w-2xl mx-auto leading-relaxed font-sans'>
                        Digital India 2026 is designed to identify and support
                        promising builds that solve localized problems at scale,
                        fostering a continuous growth network.
                    </p>
                </div>
                ...
                <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 items-stretch'>
                    ...
                    <div className='lg:col-span-2 bg-[#18181b]/60 border border-zinc-800/80 rounded-[32px] p-8 md:p-10 flex flex-col spotlight-card relative overflow-hidden group hover:border-[#9dff00]/25 transition-all duration-300'>
                        <div className='flex flex-col justify-between h-full'>
                            <div>
                                <span className='text-[10px] font-mono text-[#9dff00] uppercase tracking-widest block mb-6'>
                                    ORGANIZED BY C3 & SDC AT SNIST
                                </span>
                                <h3 className='text-2xl md:text-3xl font-light text-white tracking-tight leading-snug mb-6'>
                                    Turn innovative ideas into <br />
                                    <span className='font-semibold text-[#9dff00]'>
                                ...
                                <p className='text-zinc-300 text-sm md:text-base font-light leading-relaxed font-sans mb-6'>
                                    ...
                                    and{' '}
                                    <span className='text-white font-semibold'>
                                        Student Developers Community (SDC)
                                    </span>{' '}
                                    at{' '}
                                    <span className='text-white font-semibold'>
                                        Sreenidhi Institute of Science &
                                        Technology (SNIST), Hyderabad
                                    </span>
                                    .
                                </p>
                                <p className='text-zinc-400 text-xs sm:text-sm font-light leading-relaxed font-sans'>
                                    We aim to build an ecosystem where promising
                                    tech proposals do not end with the
                                    hackathon. Deserving ideas receive
                                    resources, mentoring, and support to grow
                            ...
                            <div className='mt-8 pt-6 border-t border-zinc-800/50 flex flex-wrap gap-x-6 gap-y-3 items-center text-[11px] font-mono text-zinc-500'>
                                <span className='flex items-center gap-2'>
                                    <Calendar className='w-3.5 h-3.5 text-[#9dff00]' />{' '}
                                    10-11 JULY 2026
                                </span>
                                ...
                                <span className='flex items-center gap-2'>
                                    <MapPin className='w-3.5 h-3.5 text-[#9dff00]' />{' '}
                                    SNIST, HYDERABAD
                                </span>
                                <span className='hidden sm:inline text-zinc-700'>
                                    •
                    ...
                    <div className='bg-[#18181b]/35 border border-zinc-800/80 rounded-[32px] p-8 flex flex-col spotlight-card hover:border-[#9dff00]/25 hover:bg-zinc-900/10 transition-all duration-300 group'>
                        <div>
                            ...
                            <p className='text-zinc-400 text-xs sm:text-sm font-light leading-relaxed font-sans'>
                                Recognition is not limited to the top three
                                winners. Multiple teams receive awards and
                                opportunities based on the quality, innovation,
                                technical execution, and impact of their
                                solutions.
                            </p>
                            <div className='mt-6 text-[10px] font-mono text-zinc-500 uppercase tracking-widest'>
                    ...
                    <div className='bg-[#18181b]/35 border border-zinc-800/80 rounded-[32px] p-8 flex flex-col spotlight-card hover:border-[#9dff00]/25 hover:bg-zinc-900/10 transition-all duration-300 group'>
                        <div>
                            ...
                            <p className='text-zinc-400 text-xs sm:text-sm font-light leading-relaxed font-sans'>
                                Our objective is to ensure that promising ideas
                                do not end with the hackathon. Selected projects
                                will continue to receive support to evolve into
                                real-world products and deployable solutions.
                            </p>
                            <div className='mt-6 text-[10px] font-mono text-zinc-500 uppercase tracking-widest'>
                                Active post-event incubation
                            </div>
                        </div>
                    </div>
                    ...
                    <div className='bg-[#18181b]/35 border border-zinc-800/80 rounded-[32px] p-8 flex flex-col spotlight-card hover:border-[#9dff00]/25 hover:bg-zinc-900/10 transition-all duration-300 group'>
                        <div>
                            ...
                            <p className='text-zinc-400 text-xs sm:text-sm font-light leading-relaxed font-sans'>
                                High-potential teams will be connected with
                                industry experts, mentors, and the Cloud
                                Community Club (C3) ecosystem for continued
                    ...
                    <div className='bg-[#18181b]/35 border border-zinc-800/80 rounded-[32px] p-8 flex flex-col spotlight-card hover:border-[#9dff00]/25 hover:bg-zinc-900/10 transition-all duration-300 group'>
                        <div>
                            ...
                            <p className='text-zinc-400 text-xs sm:text-sm font-light leading-relaxed font-sans'>
                                Apart from the ₹2,50,000 prize pool,
                                participants will have opportunities for
                                internships, mentorship, networking,
                                certificates, special recognitions, and
                                ecosystem support.
                            </p>
                            <div className='mt-6 text-[10px] font-mono text-zinc-500 uppercase tracking-widest'>
                                Prize Pool & Internships
                            </div>
                        </div>
                    </div>
                    ...
                    <div className='lg:col-span-3 bg-[#18181b]/60 border border-zinc-800/80 rounded-[32px] p-8 md:p-10 flex flex-col md:flex-row md:items-center justify-between gap-6 spotlight-card hover:border-[#9dff00]/25 transition-all duration-300 group relative overflow-hidden'>
                        <div className='flex-1 max-w-2xl'>
                            ...
                            <p className='text-zinc-400 text-xs sm:text-sm font-light leading-relaxed font-sans'>
                                We aim to create an ecosystem where innovative
                                solutions continue to grow even after the event,
                                contributing directly to India's digital
                                transformation.
                            </p>
                        </div>
                        <div className='flex items-center gap-2 self-start md:self-center shrink-0'>
                            ...
                            <div className='px-5 py-3 rounded-2xl bg-zinc-950/50 border border-zinc-900 text-center font-mono'>
                                ...
                                <div className='text-lg font-bold text-[#9dff00]'>
                                    6 Domains
                                </div>
                            </div>
                        </div>
            ...
            <section className='relative z-10 w-full max-w-7xl mx-auto px-6 md:px-12 py-20 md:py-28 border-t border-zinc-800/40'>
                <div className='text-center mb-16'>
                    ...
                    <p className='text-zinc-400 text-sm md:text-base font-light max-w-2xl mx-auto leading-relaxed'>
                        A rigorous screening process designed to filter the most
                        impactful ideas and bring the top minds to SNIST.
                    </p>
                </div>
                ...
                <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 selection-grid-anim'>
                    ...
                    <div className='selection-card-anim spotlight-card border border-zinc-800 bg-zinc-900/10 hover:border-[#9dff00]/25 rounded-3xl p-6 transition-all duration-300'>
                        <div className='text-2xl font-bold font-mono text-[#9dff00] mb-4'>
                            01.
                        </div>
                        ...
                        <p className='text-zinc-400 text-xs sm:text-sm font-light leading-relaxed'>
                            Form a team of 1–4 members, select a domain track,
                            and submit your abstract detailing the solution
                            before 5 July.
                        </p>
                    </div>
                    ...
                    <div className='selection-card-anim spotlight-card border border-zinc-800 bg-zinc-900/10 hover:border-[#9dff00]/25 rounded-3xl p-6 transition-all duration-300'>
                        ...
                        <p className='text-zinc-400 text-xs sm:text-sm font-light leading-relaxed'>
                            Our evaluation panel will shortlist the Top 100 most
                            impactful, original problem statements based on
                            localization and feasibility.
                        </p>
                    </div>
                    ...
                    <div className='selection-card-anim spotlight-card border border-zinc-800 bg-zinc-900/10 hover:border-[#9dff00]/25 rounded-3xl p-6 transition-all duration-300'>
                        ...
                        <p className='text-zinc-400 text-xs sm:text-sm font-light leading-relaxed'>
                            Invited teams will attend a physical hands-on
                            development workshop at SNIST campus on 9 July to
                            refine products.
                        </p>
                    </div>
                    ...
                    <div className='selection-card-anim spotlight-card border border-[#9dff00]/20 bg-zinc-900/20 hover:border-[#9dff00]/45 rounded-3xl p-6 transition-all duration-300 shadow-[0_0_20px_rgba(157,255,0,0.02)]'>
                        ...
                        <p className='text-zinc-400 text-xs sm:text-sm font-light leading-relaxed'>
                            Develop your idea into a working software
                            application during the 24-hour build sprint on 10–11
                            July and present to judges.
                        </p>
                    </div>
                </div>
            </section>
            ...
            <section className='relative z-10 w-full max-w-7xl mx-auto px-6 md:px-12 py-20 md:py-28 border-t border-zinc-800/40'>
                <div className='text-center mb-16'>
                    ...
                    <p className='text-zinc-400 text-sm md:text-base font-light max-w-2xl mx-auto leading-relaxed'>
                        The Digital India Hackathon 2026 consists of two stages
                        designed to discover and scale impactful tech solutions.
                    </p>
                </div>
                    ...
                    <div className='process-card-anim spotlight-card border border-zinc-800 bg-[#18181b]/35 hover:border-[#9dff00]/25 rounded-[32px] p-8 flex flex-col justify-between transition-all duration-300 group'>
                        <div>
                            <div className='flex items-center justify-between mb-6'>
                                <span className='text-xs font-mono text-[#9dff00] uppercase tracking-wider'>
                                    STAGE 1
                                </span>
                                <div className='px-3 py-1 rounded-full bg-[#9dff00]/10 border border-[#9dff00]/20 text-[#9dff00] text-[10px] font-mono font-bold'>
                                    ONLINE
                                </div>
                            </div>
                            ...
                            <ul className='space-y-4 text-zinc-400 text-sm font-light'>
                                <li className='flex items-start gap-3'>
                                    <span className='text-[#9dff00] mt-1.5'>
                                        •
                                    </span>
                                    <span>
                                        Registration Fee:{' '}
                                        <strong className='text-white'>
                                            ₹99 per team submission
                                        </strong>
                                    </span>
                                </li>
                                <li className='flex items-start gap-3'>
                                    ...
                                    <span>
                                        Teams submit their idea through the
                                        online portal before the submission
                                        deadline.
                                    </span>
                                </li>
                                <li className='flex items-start gap-3'>
                                    ...
                                    <span>
                                        All submissions will be evaluated by the
                                        judging panel.
                                    </span>
                                </li>
                            </ul>
                    ...
                    <div className='process-card-anim spotlight-card border border-zinc-800 bg-[#18181b]/35 hover:border-[#9dff00]/25 rounded-[32px] p-8 flex flex-col justify-between transition-all duration-300 group'>
                        <div>
                            <div className='flex items-center justify-between mb-6'>
                                ...
                                <div className='px-3 py-1 rounded-full bg-blue-500/10 border border-blue-500/20 text-blue-400 text-[10px] font-mono font-bold'>
                                    OFFLINE
                                </div>
                            </div>
                            <h3 className='text-2xl font-light text-white mb-4'>
                            ...
                            <ul className='space-y-4 text-zinc-400 text-sm font-light'>
                                ...
                                <li className='flex items-start gap-3'>
                                    ...
                                    <span>
                                        Shortlisted participants must complete
                                        the payment to confirm participation.
                                    </span>
                                </li>
                            </ul>
                        </div>
                        ...
                        <div>
                            ...
                            <div className='space-y-6'>
                                <div className='p-4 rounded-2xl bg-zinc-950/50 border border-zinc-900'>
                                    <div className='text-xs text-[#9dff00] font-mono uppercase tracking-wider mb-1'>
                                        Shortlisting Criterion
                                    </div>
                                    <p className='text-zinc-300 text-xs font-light leading-relaxed'>
                                        <strong>Top 100 teams</strong> will be
                                        shortlisted to participate in the
                                        offline hackathon at{' '}
                                        <strong>
                                            Sreenidhi Institute of Science and
                                            Technology (SNIST)
                                        </strong>
                                        . Shortlisted teams will be notified via
                                        email.
                                    </p>
                                </div>
                                <div className='space-y-3'>
                                    ...
                                    <div className='flex items-center justify-between text-xs pt-1'>
                                        <span className='text-zinc-400'>
                                            Offline Hackathon (per participant)
                                        </span>
                                        <span className='text-white font-mono font-bold'>
                                            ₹300
                                        </span>
                    ...
                    <div className='flex flex-col md:flex-row md:items-center justify-between gap-6 relative z-10'>
                        <div className='flex-1 max-w-3xl text-left'>
                            <div className='flex items-center justify-start gap-2 mb-4'>
                                ...
                                <span className='text-xs font-mono text-[#9dff00] uppercase tracking-widest'>
                                    REFERRAL PROGRAM
                                </span>
                            </div>
                            <h3 className='text-xl md:text-2xl font-light text-white mb-3'>
                                Refer & Get{' '}
                                <span className='text-[#9dff00] font-normal'>
                                    Free Hackathon Passes
                                </span>
                            </h3>
                            <p className='text-zinc-400 text-sm font-light leading-relaxed font-sans'>
                                Share your unique referral code (generated upon
                                registration) with other innovators. Teams that
                                secure{' '}
                                <strong className='text-white'>
                                    10 successful referrals
                                </strong>{' '}
                                will receive{' '}
                                <strong className='text-[#9dff00]'>
                                    FREE passes
                                </strong>{' '}
                                (the offline participation fee will be waived
                                entirely for the whole team).
                            </p>
                        </div>
                        <div className='flex items-center gap-4 self-start md:self-center shrink-0'>
                            ...
                            <div className='px-5 py-3 rounded-2xl bg-[#9dff00]/10 border border-[#9dff00]/20 text-center font-mono'>
                                <div className='text-xs text-[#9dff00]'>
                                    REWARD
                                </div>
                                <div className='text-lg font-bold text-[#9dff00]'>
                                    Free Pass
            ...
            <section
                id='details'
                className='relative z-10 w-full max-w-7xl mx-auto px-6 md:px-12 py-20 md:py-28 border-t border-zinc-800/40'
            >
                <div className='text-center mb-16'>
                    <h2 className='text-3xl md:text-5xl font-light text-white tracking-tight mb-4'>
                        Event{' '}
                    ...
                    <p className='text-zinc-400 text-sm md:text-base font-light max-w-2xl mx-auto leading-relaxed'>
                        Track the major program milestones. Shortlisted teams
                        will transition from online submissions to on-campus
                        development.
                    </p>
                </div>
                ...
                <div className='relative timeline-container-anim max-w-4xl mx-auto'>
                    ...
                    <div
                        className='absolute left-4 md:left-1/2 w-[1px] bg-[#9dff00] origin-top scale-y-0 timeline-progress-line shadow-[0_0_10px_#9dff00]'
                    ...
                    <div className='space-y-12'>
                        ...
                        <div className='timeline-item-anim relative flex flex-col md:flex-row items-start md:justify-between group'>
                            ...
                            <div className='timeline-content pl-12 md:pl-0 md:w-[45%] text-left md:text-right'>
                                <div className='text-xs font-mono text-[#9dff00] mb-2 uppercase tracking-wider'>
                                    UNTIL 5 JULY
                                </div>
                                ...
                                <p className='text-zinc-400 text-sm font-light leading-relaxed'>
                                    Pitch your innovative idea abstract and
                                    choose your domain track. Valid submissions
                                    are awarded a certificate and evaluated for
                                    the final event.
                                </p>
                            </div>
                        ...
                        <div className='timeline-item-anim relative flex flex-col md:flex-row items-start md:justify-between group'>
                            ...
                            <div className='timeline-content pl-12 md:pl-0 md:w-[45%] text-left'>
                                ...
                                <p className='text-zinc-400 text-sm font-light leading-relaxed mb-3'>
                                    A hands-on physical workshop designed to
                                    prepare teams. We cover cloud APIs, FOSS
                                    implementation, smart automation
                                ...
                                <div className='flex flex-wrap gap-2'>
                                    {[
                                        ...
                                        <span
                                            key={t}
                                            className='text-[9px] font-mono px-2 py-0.5 rounded border border-zinc-800 bg-zinc-900/60 text-zinc-400'
                                        >
                                            {t}
                                        </span>
                                    ))}
                        ...
                        <div className='timeline-item-anim relative flex flex-col md:flex-row items-start md:justify-between group'>
                            ...
                            <div className='timeline-content pl-12 md:pl-0 md:w-[45%] text-left md:text-right'>
                                ...
                                <p className='text-zinc-400 text-sm font-light leading-relaxed'>
                                    Sprint physically at SNIST to transform your
                                    proposal into a fully working software
                                    solution. Collaborate with on-site mentors
            ...
            <section className='relative z-10 w-full max-w-7xl mx-auto px-6 md:px-12 py-20 md:py-28 border-t border-zinc-800/40'>
                <div className='text-center mb-16'>
                    ...
                    <p className='text-zinc-400 text-sm md:text-base font-light max-w-2xl mx-auto leading-relaxed font-sans'>
                        Compete for a grand prize pool, secure valuable
                        internship opportunities, and receive guidance from
                        industry mentors.
                    </p>
                </div>
                ...
                <div className='grid grid-cols-1 lg:grid-cols-3 gap-8 items-stretch highlight-grid-anim'>
                    ...
                    <div className='highlight-card-anim spotlight-card lg:col-span-1 border border-[#9dff00]/25 bg-gradient-to-b from-[#18181b] to-[#0d0d0e] rounded-3xl p-8 flex flex-col justify-between shadow-[0_0_40px_rgba(157,255,0,0.03)] relative overflow-hidden group'>
                        ...
                        <div>
                            ...
                            <span className='text-xs font-mono text-[#9dff00] uppercase tracking-widest block mb-1'>
                                CASH PRIZES & MORE
                            </span>
                            ...
                            <p className='text-zinc-400 text-xs sm:text-sm font-light leading-relaxed'>
                                Exciting cash rewards, exclusive internships,
                                custom swag packages, and recognition
                                certificates await top-performing teams.
                            </p>
                        </div>
                        <div className='mt-8 pt-6 border-t border-zinc-800/60 text-left'>
                            ...
                            <div className='text-white text-xs font-mono font-bold uppercase tracking-wider'>
                                Winner categories revealed soon
                            </div>
                        </div>
                    </div>
                    ...
                    <div className='lg:col-span-2 grid grid-cols-1 sm:grid-cols-2 gap-6'>
                        ...
                        <div className='highlight-card-anim spotlight-card border border-zinc-800/80 bg-zinc-900/10 hover:border-zinc-700 rounded-3xl p-6 flex flex-col justify-between transition-all group'>
                            ...
                            <div className='mt-8'>
                                ...
                                <p className='text-zinc-400 text-xs sm:text-sm font-light leading-relaxed'>
                                    Every participant who submits a valid idea
                                    abstract and completes the verification will
                                    receive an official Participation
                                    Certificate.
                                </p>
                            </div>
                        </div>
                        ...
                        <div className='highlight-card-anim spotlight-card border border-zinc-800/80 bg-zinc-900/10 hover:border-zinc-700 rounded-3xl p-6 flex flex-col justify-between transition-all group'>
                            ...
                            <div className='mt-8'>
                                ...
                                <p className='text-zinc-400 text-xs sm:text-sm font-light leading-relaxed'>
                                    ...
                                    considered for direct internship
                                    opportunities with partner organizations and
                                    growing startups.
                                </p>
                            </div>
                        </div>
                        ...
                        <div className='highlight-card-anim spotlight-card border border-zinc-800/80 bg-zinc-900/10 hover:border-zinc-700 rounded-3xl p-6 flex flex-col justify-between transition-all group'>
                            ...
                            <div className='mt-8'>
                                ...
                                <p className='text-zinc-400 text-xs sm:text-sm font-light leading-relaxed'>
                                    Gain hands-on mentoring from industry
                                    leaders specializing in AI orchestration,
                                    cloud computing, and product design.
                                </p>
                            </div>
                        </div>
                        ...
                        <div className='highlight-card-anim spotlight-card border border-zinc-800/80 bg-zinc-900/10 hover:border-zinc-700 rounded-3xl p-6 flex flex-col justify-between transition-all group'>
                            ...
                            <div className='mt-8'>
                                ...
                                <p className='text-zinc-400 text-xs sm:text-sm font-light leading-relaxed'>
                                    Present your builds directly to hiring
                                    managers, founders, and community leads
                                    seeking technical talent.
                                </p>
                            </div>
                        </div>
                    </div>
            ...
            <section className='relative z-10 w-full max-w-7xl mx-auto px-6 md:px-12 py-20 md:py-28 border-t border-zinc-800/40 bg-black/10'>
                <div className='text-center mb-16'>
                    ...
                    <p className='text-zinc-400 text-sm md:text-base font-light max-w-2xl mx-auto leading-relaxed'>
                        Form a team, build actual working solutions, and unlock
                        exclusive rewards.
                    </p>
                </div>
                ...
                <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 highlight-grid-anim'>
                    <div className='highlight-card-anim spotlight-card border border-zinc-800/80 bg-zinc-900/5 hover:border-zinc-700 rounded-2xl p-6 transition-all duration-300'>
                        ...
                        <p className='text-zinc-400 text-xs leading-relaxed'>
                            Develop localized products designed to optimize
                            resource allocation for micro-businesses and public
                            administrations.
                        </p>
                    </div>
                    ...
                    <div className='highlight-card-anim spotlight-card border border-zinc-800/80 bg-zinc-900/5 hover:border-zinc-700 rounded-2xl p-6 transition-all duration-300'>
                        ...
                        <p className='text-zinc-400 text-xs leading-relaxed'>
                            Learn the disciplines of rapid MVP creation, API
                            mockups, and deployment pipelines during a 24-hour
                            build sprint.
                        </p>
                    </div>
                    ...
                    <div className='highlight-card-anim spotlight-card border border-zinc-800/80 bg-zinc-900/5 hover:border-zinc-700 rounded-2xl p-6 transition-all duration-300'>
                        ...
                        <p className='text-zinc-400 text-xs leading-relaxed'>
                            Connect with fellow developers, UI designers, and
                            startup innovators from multiple institutions across
                            India.
                        </p>
                    </div>
                    ...
                    <div className='highlight-card-anim spotlight-card border border-zinc-800/80 bg-zinc-900/5 hover:border-zinc-700 rounded-2xl p-6 transition-all duration-300'>
                        ...
                        <p className='text-zinc-400 text-xs leading-relaxed'>
                            Acquire branded T-shirts, premium developer sticker
                            packs, and limited edition community merchandise.
                        </p>
                    </div>
                </div>
            </section>
            ...
            <section
                ref={formSectionRef}
                id='register'
                className='relative z-10 w-full max-w-[1440px] mx-auto px-6 md:px-12 py-20 md:py-28 border-t border-zinc-800/40'
            >
                <div className='text-center mb-12'>
                    ...
                    <p className='text-zinc-400 text-sm md:text-base font-light max-w-2xl mx-auto leading-relaxed'>
                        Submit your details, select your domain track, describe
                        your idea proposal, and complete payment verification.
                    </p>
                </div>
                    ...
                    {[
                        ...
                        return (
                            <div
                                ...
                                onClick={() =>
                                    activeStep > stepNum &&
                                    setActiveStep(stepNum as 1 | 2 | 3 | 4)
                                }
                            ...
                            >
                                ...
                                {idx > 0 && (
                                    <div
                                        className={`absolute right-[50%] top-3 -translate-y-1/2 w-full h-[1px] -z-10 transition-colors duration-300 ${
                                            activeStep >= stepNum
                                                ? 'bg-[#9dff00]/50'
                                                : 'bg-zinc-800/60'
                                        }`}
                                ...
                                <div
                                    className={`w-6 h-6 rounded-full flex items-center justify-center border font-bold text-[9px] transition-all duration-300 ${
                                        isActive
                                ...
                                <span
                                    className={`text-[8px] sm:text-[9px] font-sans font-bold tracking-wider transition-colors duration-300 ${
                                        isActive
                                            ? 'text-[#9dff00]'
                                            : isCompleted
                                              ? 'text-zinc-300'
                ...
                <div className='flex w-full flex-col lg:flex-row items-stretch justify-center gap-6 max-w-[1440px] h-auto lg:h-[600px] relative'>
                    <div
                        ref={containerRef}
                        className='flex-1 flex flex-col lg:flex-row gap-4 w-full h-auto lg:h-full items-stretch select-none'
                    >
                        {/* Step 1 Card: PROFILE */}
                        <div
                            key='step-1'
                            ...
                            className={`relative border border-zinc-800/40 ${isTeamSizeOpen ? 'overflow-visible' : 'overflow-hidden'} flex-col justify-between h-auto lg:h-full w-full lg:w-auto bg-[#18181b] text-[#f4f4f5] rounded-[32px] p-6 sm:p-8 lg:p-10 ${
                            ...
                            {activeStep > 1 ? (
                                ...
                                <div className='fade-in-content flex flex-col justify-between h-full w-full'>
                                    <div className='flex-1 flex flex-col min-h-0'>
                                        <div className='text-xs font-bold text-zinc-500 font-mono mb-4'>
                                            01.
                                        </div>
                                        <h2 className='text-3xl font-light text-[#9dff00] mb-6 tracking-tight font-sans'>
                                        ...
                                        <div className='flex-1 overflow-visible pr-2 space-y-6 lg:space-y-8 mb-6'>
                                            <div className='grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-8'>
                                                <div>
                                                    ...
                                                    />
                                                    <span className='text-[10px] text-zinc-500 font-mono uppercase tracking-widest mt-1.5 block'>
                                                        Team Leader Name
                                                    </span>
                                                    {errors.leaderName && (
                                                            ...
                                                            {[1, 2, 3, 4].map(
                                                                (size) => (
                                                                    ...
                                                                    >
                                                                        ...
                                                                        {teamSize ===
                                                                            size && (
                                                                            <Check className='w-4 h-4 text-[#9dff00]' />
                                                                        )}
                                                                    </button>
                                                                )
                                                            )}
                            ...
                            {activeStep === 1 ? (
                                ...
                                <div className='fade-in-content flex flex-col justify-between h-full w-full'>
                                    <div className='flex-1 flex flex-col min-h-0'>
                                        ...
                                        <h2 className='text-3xl font-light text-[#9dff00] mb-6 tracking-tight'>
                                            ...
                                            <div className='space-y-6'>
                                                {teamSize === 1 ? (
                                                    <div className='flex flex-col items-center justify-center py-12 text-center border border-dashed border-zinc-800 rounded-2xl p-6 bg-zinc-950/20'>
                                                        ...
                                                        <h4 className='text-zinc-300 text-sm font-medium mb-1'>
                                                            Solo Participation
                                                        </h4>
                                                    ...
                                                    Array.from({
                                                        ...
                                                        >
                                                            <div>
                                                                ...
                                                                />
                                                                <span className='text-[9px] text-zinc-500 font-mono uppercase tracking-widest mt-1 block'>
                                                                    Member{' '}
                                                                    {i + 2} Name
                                                                </span>
                                                                {errors[
                        ...
                        <div
                            ...
                            className={`relative border border-zinc-800/40 ${isDomainOpen ? 'overflow-visible' : 'overflow-hidden'} flex-col justify-between h-auto lg:h-full w-full lg:w-auto bg-[#18181b] text-[#f4f4f5] rounded-[32px] p-6 sm:p-8 lg:p-10 ${
                            ...
                            {activeStep < 3 ? (
                                ...
                                <div className='fade-in-content flex flex-col justify-between h-full w-full'>
                                    <div className='flex-1 flex flex-col min-h-0'>
                                        ...
                                        <h2 className='text-3xl font-light text-[#9dff00] mb-6 tracking-tight'>
                                            Select domain & describe solution.
                                        </h2>
                                        ...
                                        <div className='flex-1 overflow-visible pr-2 space-y-6 mb-6'>
                                            ...
                                            <div>
                                                <textarea
                                                    ...
                                                    className={`w-full text-xl font-light py-4 border-b ${
                                                        errors.idea
                                                            ? 'border-red-400 focus:border-red-500'
                                                            : 'border-zinc-800 focus:border-zinc-400'
                                                ...
                                                <div className='flex justify-between items-center mt-2.5'>
                                                    <span className='text-[10px] text-zinc-500 font-mono uppercase tracking-widest'>
                                                        Idea Proposal Details
                                                    </span>
                                                    <span
                                                        className={`text-[10px] font-mono ${idea.length < 150 ? 'text-zinc-500' : 'text-[#9dff00] font-semibold'}`}
                                                    >
                                                        {idea.length}/150 min
                                                        characters
                                                    </span>
                            ...
                            {activeStep < 4 ? (
                                ...
                                <div className='success-content fade-in-content flex flex-col justify-between h-full w-full'>
                                    <div className='flex-1 overflow-y-auto pr-2 custom-scrollbar'>
                                        <div className='text-xs font-bold text-zinc-500 font-mono mb-2'>
                                            04.
                                        </div>
                                        <h2 className='text-2xl font-light text-[#9dff00] mb-4 tracking-tight font-sans'>
                                        ...
                                        <div className='bg-zinc-900/40 border border-zinc-800 p-5 rounded-2xl space-y-4'>
                                            ...
                                            <div className='grid grid-cols-3 gap-2'>
                                                <div className='bg-black/40 border border-zinc-900 p-3 rounded-xl text-center'>
                                                    <div className='text-zinc-500 text-[8px] font-mono uppercase tracking-wider mb-0.5'>
                                                        Points
                                                    </div>
                                                    <div className='text-sm font-bold font-mono text-white'>
                                                        {dashboardData?.referralPoints ??
                                                            0}
                                                    </div>
                                                </div>
                                                <div className='bg-black/40 border border-zinc-900 p-3 rounded-xl text-center'>
                                                    ...
                                                    <div className='text-sm font-bold font-mono text-[#9dff00]'>
                                                        #
                                            ...
                                            <div className='space-y-1.5 pt-2'>
                                                <label className='text-[9px] text-zinc-500 font-mono uppercase tracking-widest block'>
                                                    Your Referral Link
                                                </label>
                                                <div className='text-[10px] font-mono bg-black/60 p-2.5 rounded-xl border border-zinc-800 text-zinc-300 break-all select-all flex items-center justify-between'>
                                                    <span>
                                                        {typeof window !==
                                                        'undefined'
                                                            ? `${window.location.origin}/events/digitalindia?ref=${myReferralCode}`
                                                            : `https://domain.com/events/digitalindia?ref=${myReferralCode}`}
                                                    </span>
                                                </div>
                                            ...
                                            <div className='pt-2 border-t border-zinc-900/60'>
                                                <div className='text-[9px] font-mono text-zinc-500 mb-1.5 uppercase tracking-wide'>
                                                    Share Link
                                                </div>
                                                        ...
                                                        return (
                                                            <>
                                                                <a
                                                                    ...
                                                                    className='bg-green-600 hover:bg-green-700 text-white px-2.5 py-1 rounded text-[10px] font-semibold transition-colors'
                                                                >
                                                                    WhatsApp
                                                                </a>
                                                                <a
                                                                    ...
                                                                    target='_blank'
                                                                    rel='noopener noreferrer'
                                                                    className='bg-sky-600 hover:bg-sky-700 text-white px-2.5 py-1 rounded text-[10px] font-semibold transition-colors'
                                                                >
                                                                    Telegram
                                                                </a>
                                                                <a
                                                                    ...
                                                                    target='_blank'
                                                                    rel='noopener noreferrer'
                                                                    className='bg-neutral-900 hover:bg-neutral-800 border border-neutral-800 text-white px-2.5 py-1 rounded text-[10px] font-semibold transition-colors'
                                                                >
                                                                    X
                                                                </a>
                                                                <a
                                                                    ...
                                                                    target='_blank'
                                                                    rel='noopener noreferrer'
                                                                    className='bg-blue-700 hover:bg-blue-800 text-white px-2.5 py-1 rounded text-[10px] font-semibold transition-colors'
                                                                >
                                                                    LinkedIn
                                                                </a>
                                                            </>
                                ...
                                <form
                                    ...
                                    className='fade-in-content flex flex-col justify-between h-full w-full'
                                >
                                    <div className='flex-1 overflow-y-auto pr-2 custom-scrollbar space-y-5'>
                                        <div>
                                            ...
                                            <h2 className='text-3xl font-light text-[#9dff00] mb-4 tracking-tight'>
                                        ...
                                        <div className='bg-zinc-900/50 p-5 rounded-2xl border border-zinc-800 grid grid-cols-1 lg:grid-cols-2 gap-6'>
                                            <div>
                                                ...
                                                <p className='text-xs text-zinc-400 leading-relaxed mb-4'>
                                                    Please scan the UPI QR code
                                                    to complete your payment of
                                                    ₹{upiAmount}. Note your UPI
                                                    Transaction ID/UTR once
                                                    finished.
                                                </p>
                                                        ...
                                                        >
                                                            {copied ? (
                                                                <>
                                                                    ...
                                                                    <span className='text-[#9dff00] font-medium'>
                                                                        Copied!
                                                                    </span>
                                                                </>
                                                            ) : (
                                        ...
                                        <div className='grid grid-cols-1 lg:grid-cols-2 gap-6 pt-1'>
                                            <div>
                                                <label className='text-[10px] text-zinc-500 font-mono uppercase tracking-widest mb-1.5 block'>
                                                    UTR / Transaction ID
                                                </label>
                                                <input
                                                    type='text'
                                        ...
                                        {apiError && (
                                            <p className='text-red-400 text-xs text-center border border-red-950/40 bg-red-950/25 rounded-xl py-3 px-4 font-mono'>
                                                {apiError}
                                            </p>
                                        )}
                                    </div>
                                    ...
                                    <div className='flex justify-between items-center mt-4 pt-4 border-t border-zinc-800'>
                                        <div className='text-[9px] text-zinc-500 max-w-[280px] leading-tight font-mono uppercase'></div>
                                        <div className='flex gap-3'>
                                            <button
                                                type='button'
            ...
            <section
                ...
                className='relative z-10 w-full max-w-5xl mx-auto px-6 py-12 md:py-16 border-t border-zinc-800/40'
            >
                <div className='bg-zinc-900/30 border border-zinc-800/80 rounded-[32px] p-6 md:p-10 spotlight-card'>
                    ...
                    <p className='text-zinc-400 text-xs sm:text-sm font-light leading-relaxed mb-8 max-w-2xl font-sans'>
                        Track your successful referrals, view your rank on the
                        leaderboard, and claim your free passes once you reach
                        10 successful referrals!
                    </p>
                        ...
                        <div className='space-y-8'>
                            <div className='grid grid-cols-2 md:grid-cols-4 gap-4'>
                                <div className='bg-zinc-900/60 border border-zinc-800/50 p-5 rounded-2xl text-center'>
                                    ...
                                    <div className='text-2xl font-bold font-mono text-[#9dff00]'>
                                ...
                                <div className='bg-zinc-900/60 border border-zinc-800/50 p-5 rounded-2xl text-center'>
                                    ...
                                    <div className='text-2xl font-bold font-mono text-white'>
                                        {dashboardData.successfulReferrals
                                            ?.length || 0}
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className='bg-zinc-950/40 border border-zinc-900 p-6 rounded-2xl space-y-4'>
                                <div className='flex flex-col sm:flex-row sm:items-center justify-between gap-4'>
                                    <div className='flex-1'>
                                        <label className='text-[9px] text-zinc-500 font-mono uppercase tracking-widest block mb-1'>
                                        ...
                                        <div className='text-xs font-mono bg-black/60 p-3 rounded-xl border border-zinc-800 text-zinc-300 break-all select-all'>
                                            {typeof window !== 'undefined'
                                                ? `${window.location.origin}/events/digitalindia?ref=${dashboardData.referralCode}`
                                                : `https://domain.com/events/digitalindia?ref=${dashboardData.referralCode}`}
                                        </div>
                                    </div>
                                ...
                                <div className='pt-2 border-t border-zinc-900/60'>
                                    <div className='text-xs font-mono text-zinc-400 mb-2 uppercase tracking-wide'>
                                            ...
                                            return (
                                                <>
                                                    <a
                                                        ...
                                                        target='_blank'
                                                        rel='noopener noreferrer'
                                                        className='bg-green-650 hover:bg-green-700 text-white px-3.5 py-2 rounded-xl text-xs font-semibold flex items-center gap-1.5 transition-colors'
                                                    >
                                                        WhatsApp
                                                    </a>
                                                    <a
                                                        ...
                                                        target='_blank'
                                                        rel='noopener noreferrer'
                                                        className='bg-sky-600 hover:bg-sky-700 text-white px-3.5 py-2 rounded-xl text-xs font-semibold flex items-center gap-1.5 transition-colors'
                                                    >
                                                        Telegram
                                                    </a>
                                                    <a
                                                        ...
                                                        target='_blank'
                                                        rel='noopener noreferrer'
                                                        className='bg-neutral-900 hover:bg-neutral-800 border border-neutral-800 text-white px-3.5 py-2 rounded-xl text-xs font-semibold flex items-center gap-1.5 transition-colors'
                                                    >
                                                        X
                                                    </a>
                                                    <a
                                                        ...
                                                        target='_blank'
                                                        rel='noopener noreferrer'
                                                        className='bg-blue-700 hover:bg-blue-800 text-white px-3.5 py-2 rounded-xl text-xs font-semibold flex items-center gap-1.5 transition-colors'
                                                    >
                                                        LinkedIn
                                                    </a>
                                                </>
                            ...
                            {dashboardData.successfulReferrals &&
                                dashboardData.successfulReferrals.length >
                                    0 && (
                                    <div className='mt-6'>
                                        <h4 className='text-xs font-mono text-zinc-400 mb-2 uppercase tracking-wide'>
                                            Your Referrals (
                                            {
                                                dashboardData
                                        ...
                                        <div className='grid grid-cols-1 sm:grid-cols-2 gap-2 max-h-40 overflow-y-auto pr-2 custom-scrollbar'>
                                            {dashboardData.successfulReferrals.map(
                                                (
                                                    refTeam: any,
                                                    index: number
                                                ) => (
                                                    <div
                                                        key={index}
                                                        className='bg-zinc-950/30 border border-zinc-900/60 p-3 rounded-xl flex justify-between items-center text-xs font-sans'
                                                    >
                                                        <span className='text-white font-medium'>
                                                            {refTeam.teamName}
                                                        </span>
                ...
                <div className='bg-zinc-900/30 border border-zinc-800/80 rounded-[32px] p-6 md:p-10'>
                    <div className='flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8'>
                        <div>
                            ...
                            <p className='text-zinc-500 text-xs font-mono uppercase tracking-widest'>
                                Compete and earn free hackathon passes
                            </p>
                        </div>
                    ...
                    <div className='bg-[#9dff00]/10 border border-[#9dff00]/20 rounded-2xl p-4 mb-6 text-center shadow-[0_0_20px_rgba(157,255,0,0.05)]'>
                        <p className='text-xs sm:text-sm font-semibold text-[#9dff00] flex items-center justify-center gap-2 font-sans'>
                            <Trophy className='w-4 h-4 text-[#9dff00] shrink-0' />{' '}
                            Reach 10 Successful Referrals and Earn FREE
                            Hackathon Passes!
                        </p>
                    </div>
                        ...
                        <table className='w-full text-left border-collapse text-xs sm:text-sm font-sans'>
                            <thead>
                                <tr className='bg-zinc-900/50 border-b border-zinc-800/80'>
                                    <th className='px-6 py-4 text-zinc-400 font-mono text-xs uppercase tracking-wider'>
                                        Rank
                                    </th>
                                    <th className='px-6 py-4 text-zinc-400 font-mono text-xs uppercase tracking-wider'>
                                        Team Name
                                    </th>
                                    <th className='px-6 py-4 text-zinc-400 font-mono text-xs uppercase tracking-wider text-right'>
                                        Referral Points
                                    </th>
                                </tr>
                            </thead>
                            <tbody>
                                ...
                                leaderboardData.length === 0 ? (
                                    ...
                                    <tr>
                                        <td
                                            colSpan={3}
                                            className='px-6 py-8 text-center text-zinc-500 font-mono text-xs'
                                        >
                                    ...
                                    leaderboardData.map((team) => {
                                        ...
                                        return (
                                            ...
                                            >
                                                <td className='px-6 py-4 font-mono font-bold'>
                                                    {isGold ? (
                                                        <span className='inline-flex items-center gap-1.5 text-yellow-500'>
                                                            <Medal className='w-4 h-4 text-yellow-550 shrink-0' />{' '}
                                                ...
                                                <td
                                                    className={`px-6 py-4 font-sans font-medium ${isTop10 ? 'text-white' : 'text-zinc-400'}`}
                                                >
                                                    {team.teamName}
                                                    {isTop10 && (
                ...
                <div className='grid grid-cols-1 lg:grid-cols-2 gap-12 items-start'>
                    ...
                    <div>
                        ...
                        <div className='space-y-4'>
                            {[
                                {
                                    title: 'Original Submissions Only',
                                    desc: 'All developed work must be original and built during the offline hackathon. Copied templates result in immediate disqualification.',
                                },
                                {
                                    title: 'Jury Decision Finality',
                                    desc: 'The decisions of the evaluation jury and organizing committee are final and binding in all cases.',
                                },
                            ].map((rule, idx) => (
                                <div
                                    key={idx}
                                    className='flex gap-4 items-start pb-4 border-b border-zinc-800/40 last:border-0 last:pb-0'
                                >
                                    ...
                                    <div>
                                        <h4 className='text-white text-sm font-medium mb-1 font-sans'>
                                            {rule.title}
                                        </h4>
                        ...
                        <div className='space-y-3'>
                            {faqs.map((faq, idx) => {
                                ...
                                return (
                                    <div
                                        key={idx}
                                        className='border border-zinc-800/80 bg-zinc-900/5 rounded-2xl overflow-hidden transition-all duration-300'
                                    >
                                        <button
                                            type='button'
                                            onClick={() =>
                                        ...
                                        >
                                            <span className='text-xs sm:text-sm font-medium pr-4'>
                                                {faq.q}
                                            </span>
                                            <ChevronDown
                                                className={`w-4 h-4 text-zinc-500 shrink-0 transition-transform duration-300 ${isOpen ? 'rotate-180 text-[#9dff00]' : ''}`}
                                            />
                                        </button>
