...
describe('Referral System Database Integration Tests', () => {
    ...
    test('1. Generate referralCode on Team A registration', async () => {
        const teamA = new DigitalIndiaSubmission({
            ...
            teamMembers: [
                {
                    name: 'Member A2',
                    email: `a2_test_${testTimestamp}@example.com`,
                },
                {
                    name: 'Member A3',
                    email: `a3_test_${testTimestamp}@example.com`,
                },
            ],
        })
    ...
    test('2. Award referral point immediately upon Team B registration with Team A code', async () => {
        const teamA = await DigitalIndiaSubmission.findOne({
            email: testEmailA,
        })
        assert.ok(teamA)
        ...
        const teamB = new DigitalIndiaSubmission({
            ...
            teamMembers: [
                {
                    name: 'Member B2',
                    email: `b2_test_${testTimestamp}@example.com`,
                },
                {
                    name: 'Member B3',
                    email: `b3_test_${testTimestamp}@example.com`,
                },
            ],
        ...
        const savedLog = await DigitalIndiaReferral.findOne({
            referredTeamId: savedTeamB._id,
        })
        assert.ok(savedLog)
    ...
    test('3. Enforce unique teamName constraint', async () => {
        ...
        try {
            ...
            assert.fail(
                'Should have failed due to duplicate teamName constraint'
            )
        } catch (err: any) {
            assert.ok(
                err.code === 11000 ||
    ...
    test('4. Leaderboard sorting with tie-breaker priority', async () => {
        ...
        teams.sort((a, b) => {
            ...
            return (
                new Date(a.lastPointEarnedAt).getTime() -
                new Date(b.lastPointEarnedAt).getTime()
            )
        })
