...
function StatCard({
    ...
    const colorMap = {
        blue: { bg: 'bg-[#8be9fd]/10', border: 'border-[#8be9fd]/30', icon: 'text-[#8be9fd]', value: 'text-[#8be9fd]' },
...
export default function DigitalIndiaAdminDashboard() {
    ...
    const apiUrl = activeSubTab === 'submissions'
    ...
    const handleAccept = async (id: string) => {
        if (!confirm('Are you sure you want to accept this candidate? They will be moved to the Accepted Participants list and notified via email.')) {
            return
        }
        setAcceptingId(id)
        try {
            ...
            <main className="max-w-[1400px] mx-auto px-4 sm:px-6 py-6">
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
                    ...
                    <StatCard
                        ...
                        value={(globalStats?.digitalIndia?.total ?? 0) + (globalStats?.digitalIndia?.accepted ?? 0)}
                ...
                <div className="bg-[#0d0e12] border border-[#6272a4]/20 rounded-xl p-5">
                    ...
                    <div className="flex items-center justify-between mb-5 pb-4 border-b border-[#6272a4]/20 flex-wrap gap-4">
                        <div className="flex items-center gap-4">
                            ...
                            >
                                Submissions ({globalStats?.digitalIndia?.total ?? 0})
                            </button>
                            <button
                            ...
                            >
                                Accepted Participants ({globalStats?.digitalIndia?.accepted ?? 0})
                            </button>
                        </div>
                        <div className="flex items-center gap-2 text-xs font-mono text-[#6272a4]">
                            <Filter size={12} />
                            <span>{activeSubTab === 'submissions' ? 'Submissions list' : 'Accepted list'}</span>
                        </div>
                    </div>
                            ...
                            <table className="w-full text-sm">
                                <thead>
                                    <tr className="bg-[#1e1f29] border-b border-[#6272a4]/20">
                                        ...
                                        {activeSubTab === 'submissions' ? (
                                            <>
                                                <th className="text-left px-4 py-3 text-[#6272a4] font-mono text-xs uppercase tracking-wider">Status</th>
                                                <th className="text-left px-4 py-3 text-[#6272a4] font-mono text-xs uppercase tracking-wider">Action</th>
                                                <th className="text-left px-4 py-3 text-[#6272a4] font-mono text-xs uppercase tracking-wider">Date</th>
                                            </>
                                        ) : (
                                            <>
                                                <th className="text-left px-4 py-3 text-[#6272a4] font-mono text-xs uppercase tracking-wider">Accepted By</th>
                                                <th className="text-left px-4 py-3 text-[#6272a4] font-mono text-xs uppercase tracking-wider">Date Accepted</th>
                                            </>
                                        )}
                                    </tr>
                                </thead>
                                <tbody>
                                        ...
                                        <>
                                            {Array.from({ length: 5 }).map((_, i) => (
                                                <tr key={i} className="border-b border-[#6272a4]/10">
                                                    {Array.from({ length: activeSubTab === 'submissions' ? 8 : 7 }).map((_, j) => (
                                    ...
                                    {error && (
                                        <tr>
                                            <td colSpan={activeSubTab === 'submissions' ? 8 : 7} className="px-4 py-8 text-center">
                                                <div className="flex flex-col items-center gap-2 text-[#ff5555]">
                                                    <AlertTriangle size={20} />
                                                    <span className="font-mono text-sm">Failed to load Digital India data</span>
                                                </div>
                                            </td>
                                        </tr>
                                    )}
                                    {!isLoading && !error && data?.data.length === 0 && (
                                        <tr>
                                            <td colSpan={activeSubTab === 'submissions' ? 8 : 7} className="px-4 py-10 text-center text-[#6272a4] font-mono text-sm">
                                                No records found matching your filters.
                                            </td>
                                        </tr>
                                    )}
                                    {!isLoading && data?.data.map((sub, i) => (
                                        <tr
                                            key={sub._id}
                                        ...
                                        >
                                            ...
                                            <td className="px-4 py-3">
                                                <button
                                                    onClick={() => setSelectedScreenshot(`/api/admin/digital-india/screenshot?id=${sub._id}&accepted=${activeSubTab === 'accepted'}`)}
                                                    ...
                                                    <td className="px-4 py-3">
                                                        <div className="flex items-center gap-2">
                                                            ...
                                                            <button
                                                                ...
                                                                title={sub.paymentVerified ? 'Accept candidate and send shortlisted email' : 'Verify payment first to accept'}
                                                            >
                                                                {acceptingId === sub._id ? (
                                                                    <RefreshCw size={12} className="animate-spin" />
                                                                ) : (
                    ...
                    {data?.pagination && (
                        <div className="flex items-center justify-between mt-4 px-1">
                            <span className="text-[#6272a4] text-xs font-mono">
                                {data.pagination.total === 0 ? '0' : (page - 1) * 20 + 1}–{Math.min(page * 20, data.pagination.total)} of {data.pagination.total} records
                            </span>
                            <div className="flex items-center gap-2">
                                <button
                ...
                {selectedIdea && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
                        ...
                        >
                            ...
                            <h3 className="text-xl font-bold font-mono text-[#bd93f9] mb-4">Idea Description</h3>
