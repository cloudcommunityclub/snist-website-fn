...
function MembersTab() {
    ...
    >(`/api/admin/members?${params.toString()}`, fetcher, {
    ...
    const handleExport = async () => {
        setDownloading(true)
        try {
            const res = await fetch('/api/admin/members?export=true', {
                        ...
                        <tbody>
                            ...
                            {!isLoading &&
                                data?.data.map((member, i) => (
                                    ...
                                    >
                                        <td className='px-4 py-3 text-[#f8f8f2] font-medium max-w-[150px] truncate'>
                                            {member.name}
                                        </td>
                                        <td className='px-4 py-3 text-[#8be9fd] font-mono text-xs max-w-[180px] truncate'>
                                            {member.email}
                                        </td>
                                        ...
                                        <td className='px-4 py-3 text-[#f8f8f2] text-center'>
                                            {member.year}
                                        </td>
                                        <td className='px-4 py-3'>
                                            {member.emailSent ? (
...
function RecruitmentTab() {
    ...
    const { data, error, isLoading, mutate } = useSWR<
        PaginatedResponse<RecruitmentCandidate>
    >(`/api/admin/recruitment?${params.toString()}`, fetcher, {
        keepPreviousData: true,
    })
                            ...
                            {!isLoading &&
                                data?.data.map((candidate, i) => (
                                    <tr
                                        ...
                                        className={`border-b border-[#6272a4]/10 hover:bg-[#1e1f29]/60 transition-colors ${i % 2 === 0 ? 'bg-transparent' : 'bg-[#1e1f29]/20'}`}
                                    >
                                        <td className='px-4 py-3 text-[#f8f8f2] font-medium max-w-[140px] truncate'>
                                            {candidate.name}
                                        </td>
                                        <td className='px-4 py-3 text-[#8be9fd] font-mono text-xs max-w-[180px] truncate'>
                                            {candidate.email}
                                        ...
                                        <td className='px-4 py-3 text-[#bd93f9] font-mono text-center'>
                                            {candidate.passingOutYear}
                                        </td>
                                        <td className='px-4 py-3'>
                                            {candidate.problemUnlocked ? (
                                                ...
                                                <span className='text-[#44475a] text-xs font-mono'>
                                                    —
                                                </span>
                                            )}
                                        </td>
        ...
        <div className='min-h-screen bg-[#0d0e12] text-[#f8f8f2]'>
                ...
                <div className='max-w-[1400px] mx-auto px-4 sm:px-6 h-14 flex items-center justify-between'>
                    ...
                    <div className='flex items-center gap-3'>
                        ...
                        <div className='flex items-center gap-1.5 ml-2'>
                            ...
                            <span className='text-[#50fa7b] text-xs font-mono hidden sm:block'>
                                LIVE
                            </span>
                        </div>
                    </div>
                    ...
                    <div className='flex items-center bg-[#1e1f29] border border-[#6272a4]/30 rounded-lg p-1 gap-1'>
                        ...
                        <button
                            onClick={() =>
                                router.push('/admin/dashboard/digital_india')
                            }
            ...
            <main className='max-w-[1400px] mx-auto px-4 sm:px-6 py-6'>
                ...
                <div className='grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8'>
                    {statsLoading ? (
                        Array.from({ length: 4 }).map((_, i) => (
                            <div
                                key={i}
                                className='border border-[#6272a4]/20 rounded-xl p-5'
                            >
                ...
                <div className='bg-[#0d0e12] border border-[#6272a4]/20 rounded-xl p-5'>
                    ...
                    <div className='flex items-center justify-between mb-5 pb-4 border-b border-[#6272a4]/20'>
                        <div className='flex items-center gap-2'>
                            ...
                            <h2 className='font-mono font-bold text-[#f8f8f2] capitalize'>
                                {activeTab}
                            </h2>
                            <span className='text-[#6272a4] text-xs font-mono'>
                                //{' '}
                ...
                <div className='mt-6 text-center text-[#44475a] text-xs font-mono'>
                    C3 Admin Terminal — Unauthorized access is monitored and
                    logged.
                </div>
            </main>
        </div>
